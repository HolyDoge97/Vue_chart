...
export default {
  ...
  setup(props) {
    ...
    const drawBars = (padding, chartWidth, chartHeight, maxValue) => {
      ...
      const legendKeys = Object.keys(props.data[0]).filter(key => key.startsWith('legend_'))
      ...
      props.data.forEach((item, dataIndex) => {
        legendKeys.forEach((key, legendIndex) => {
          ...
          const x = padding + (dataIndex * groupWidth) + (legendIndex + 1) * barSpacing + (legendIndex * barWidth)
          ...
          if (legendIndex === 0) {
            ...
            ctx.font = '12px Arial'
            ctx.textAlign = 'center'
            ctx.fillText(`항목 ${dataIndex + 1}`, x + groupWidth / 2, chartHeight + 20)
          }
        })
      })
    }
