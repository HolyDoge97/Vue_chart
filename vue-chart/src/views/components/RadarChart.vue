  ...
  <div class="chart-container">
    ...
    <div class="legend" v-if="showLegend">
      <div v-for="(dataset, index) in data" :key="index" class="legend-item">
        <div class="color-box" :style="{ backgroundColor: colors[index % colors.length] }"></div>
        <div class="legend-label">{{ dataset.label || `데이터셋 ${index + 1}` }}</div>
      </div>
    </div>
  </div>
</template>
  ...
  setup(props) {
    ...
    const drawCategoryLabels = (centerX, centerY, radius) => {
      ...
      props.categories.forEach((category, index) => {
        ...
        // 텍스트 위치 조정 (각도에 따라)
        if (angle === -Math.PI / 2) { // 상단
          ctx.textBaseline = 'bottom'
        } else if (angle === Math.PI / 2) { // 하단
          ctx.textBaseline = 'top'
        } else if (angle > -Math.PI / 2 && angle < Math.PI / 2) { // 오른쪽
          ctx.textAlign = 'left'
        } else { // 왼쪽
          ctx.textAlign = 'right'
        }
